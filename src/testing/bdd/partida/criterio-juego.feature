
Feature: Implementar criterios de juego


Scenario: Como jugador quiero al selecionar la dificultad facil que me aperezca 7 globos

    Given Que he seleccionado la dificultad fácil
    Then el usuario tendra 7 globos por defecto


Scenario: Como jugador quiero al selecionar la dificultad normal que me aperezca 7 globos

    Given Que he seleccionado la dificultad normal
    Then el usuario tendra 5 globos por defecto


Scenario: Como jugador quiero al selecionar la dificultad difícil que me aperezca 7 globos

    Given Que he seleccionado la dificultad difícil
    Then el usuario tendra 3 globos por defecto


Scenario: Como sistema quiero eliminar un globo cuando el jugador inserte una letra incorrecta.

    Given Que el jugador ha seleccionado una letra
    And la letra seleccionada es erronea
    Then se elimina un globo, es decir, un intento.


Scenario: Como jugador quiero  ver mi progreso por medio de imagenes en pantalla

    Given Que el jugador escoja una letra 
    And la letra seleccionada sea erronea
    Then se actualiza el estado grafico de los globos y del jugador.

Scenario: Como jugador quiero poder pasar al siguiente nivel al completar la palabra

    Given Que el jugador complete la palabra
    And la palabra este correcta
    Then se gana la partida y permitiria iniciar una nueva partida.


Scenario: Como jugador quiero Terminar el juego(partida perdida)

    Given que el jugador se encuentre en partida
    And Que el jugador no tenga más globos, es decir, intentos
    Then se finaliza la partida.


Scenario: Como jugador quiero Terminar el juego

    Given que el jugador se encuentre en partida
    And Que el jugador no quiera continuar con la partida actual
    Then se finaliza la partida.

